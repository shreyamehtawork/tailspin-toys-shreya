...
- Every exported function in `db/` and `src/lib/` must have a TSDoc/JSDoc comment describing its purpose, every parameter (including the injectable `db`), and its return value. Document non-obvious schema or query decisions where they matter; do not add comments that merely paraphrase the implementation.
...
- Use single quotes, semicolons, trailing commas in multiline literals, and spaces inside object braces; ESLint enforces these conventions.
- Explain intent, rationale, and constraints rather than restating SQL or TypeScript. Keep comments current when changing the related data access code.
